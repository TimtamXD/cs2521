...
static int getGodLocation (HunterView hv) {
  ...
  LocationID prevLocations[TRAIL_SIZE] = giveMeTheTrail (hv, PLAYER_LORD_GODALMING, prevLocations);
...
static int getHarLocation (HunterView hv) {
  int nextLocation = UNKNOWN_LOCATION;
  LocationID prevLocations[TRAIL_SIZE] = giveMeTheTrail (hv, PLAYER_MINA_HARKER, prevLocations);
  if (giveMeTheRound (hv) == 0) {
...
static int getHelLocation (HunterView hv) {
  int nextLocation = UNKNOWN_LOCATION;
  LocationID prevLocations[TRAIL_SIZE] = giveMeTheTrail (hv, PLAYER_VAN_HELSING, prevLocations);
  if (giveMeTheRound (hv) == 0) {
    nextLocation = abbrevToID ("MU");
...
static int getSewLocation (HunterView hv) {
  int nextLocation = UNKNOWN_LOCATION;
  LocationID prevLocations[TRAIL_SIZE] = giveMeTheTrail (hv, PLAYER_DR_SEWARD, prevLocations);
  if (giveMeTheRound (hv) == 0) {
    nextLocaiton = abbrevToID ("AO");
